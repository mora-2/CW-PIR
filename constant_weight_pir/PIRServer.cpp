...
void PIRServer::prep_db(ServerParameters *q_params, bool _verbose)
{
...
Ciphertext PIRServer::constant_weight_eq(ServerParameters *q_params, vector<Ciphertext> &encrypted_query, vector<uint64_t> &__encoded_subindex)
{
...
Ciphertext PIRServer::folklore_eq(ServerParameters *q_params, vector<Ciphertext> &encrypted_query, vector<uint64_t> &__encoded_subindex)
{
    Ciphertext __temp_ciphertext;
    vector<Ciphertext> mult_operands;
    for (uint64_t i = 0; i < q_params->encoding_size; i++)
...
Ciphertext PIRServer::generate_selection_bit(ServerParameters *q_params, uint64_t _keyword, vector<Ciphertext> &encrypted_query)
{
...
void PIRServer::generate_selection_vector(ServerParameters *q_params, vector<Ciphertext> &expanded_query, vector<Ciphertext> &selection_vector, bool _verbose)
{
...
vector<Ciphertext> PIRServer::multiply_with_database(ServerParameters *q_params, uint64_t _subindex, Ciphertext &encrypted_selection_bit)
{
...
void PIRServer::inner_product(ServerParameters *q_params, vector<Ciphertext> &selection_vector, vector<Ciphertext> &encrypted_answer, bool _verbose)
{
...
void PIRServer::faster_inner_product(ServerParameters *q_params, vector<Ciphertext> &selection_vector, vector<Ciphertext> &encrypted_answer, bool _verbose)
{
...
void PIRServer::respond_pir_query(stringstream &data_stream, ServerParameters *q_params, bool _verbose)
{
    ...
    if (_verbose)
        cout << "\tNumber of Threads: " << q_params->num_threads << endl
             << "--------------------------------------------------------------------" << endl;
