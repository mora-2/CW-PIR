...
// void PIRClient::set_server(PIRServer* _server){
//     this->server_ = _server;
// }

void PIRClient::setup_crypto(size_t log_poly_modulus_degree, uint64_t prime_bitlength, bool _verbose)
{
...
void PIRClient::send_evaluation_keys()
{
...
// Common for all
vector<uint64_t> PIRClient::query_to_encoding(uint64_t _query, ClientParameters *q_params)
{
...
vector<Plaintext> PIRClient::encoding_to_pt(ClientParameters *q_params, vector<uint64_t> enc_)
{
...
void PIRClient::encrypt_and_send(vector<Plaintext> &pts, uint64_t num_pts)
{
...
vector<Plaintext> PIRClient::load_and_decrypt(ClientParameters *q_params)
{
...
vector<uint64_t> PIRClient::pts_to_vec(vector<Plaintext> &_pts, uint64_t num_pts_, uint64_t poly_degree)
{
...
void PIRClient::submit_PIR_query_with_params(uint64_t _query, ClientParameters *_q_params, bool _verbose)
{
...
// uint64_t PIRClient::submit_random_PIR_query_with_params(ClientParameters* _q_params, bool _verbose){
//     uint64_t __query = _q_params->db_->get_random_query();
//     this->submit_PIR_query_with_params(__query, _q_params, _verbose);
//     return __query;
// }

bool PIRClient::validate_response(ServerParameters *q_params, uint64_t _query_keyword, vector<uint64_t> _response, bool _verbose)
{
    bool __equal = true;
    ...
    for (uint64_t i = 0; i < coeffs.size(); i++)
    {
        ...
        cout << "response:" << _response[i] << endl;
    }
    if (_verbose)
    {
        if (__equal)
...
// // End to End
// bool PIRClient::end_to_end_PIR_query(ClientParameters* _q_params, bool _verbose){
